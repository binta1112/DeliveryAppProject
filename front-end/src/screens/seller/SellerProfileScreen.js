...
export default function SellerProfileScreen() {
  ...
  const { commerceantId } = useAppSelector((s) => s.auth);
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <LinearGradient colors={gradients.primary} style={styles.headerGradient}>
        <View style={styles.headerContent}>
          <Avatar name="Commerçant" size={80} />
          <Text style={styles.profileName}>Commerçant</Text>
          <Text style={styles.profileId}>ID: {commerceantId?.slice(0, 8) || 'Non défini'}</Text>

          <View style={styles.statsContainer}>
            <View style={styles.statBox}>
              <Ionicons name="receipt" size={20} color="#FFF" />
              <Text style={styles.statValue}>{commandes.length}</Text>
      ...
      <View style={styles.menuContainer}>
        ...
        <MenuOption icon="storefront-outline" label="Informations du commerce" onPress={() => {}} />
        <MenuOption icon="document-text-outline" label="Mes commandes" onPress={() => {}} />
        <MenuOption icon="people-outline" label="Mes clients" onPress={() => {}} />
